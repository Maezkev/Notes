  ...
  static boolean validname(String iname){ 
    if (iname.length() >= 5 && iname.length() <= 20){
      ...
      if(jmlkata.countTokens() >= 2){
        return true;
      }else System.out.println (">> Please input 2 words name."); 
       return false; 
    }else System.out.println (">> Please input name between 5 to 20 characters."); 
     return false; 
  }
  ...
  static void INC(){
    ...
    while(!validgender(dataCD[1]));
      //do{
        ...
        dataCD[2] = inputuser.next().toUpperCase();  
      //}
